...
- [ ] **1a.** Extend `WebSearchTool` model with optional `queries: list[str]` field and `model_validator` that normalizes `query` → `queries`
...
- [ ] **1b.** Update `_handle_web_search_tool` to dispatch batch queries via `asyncio.gather` over `_topic_search`
...
- [ ] **1c.** Refactor budget accounting: `_handle_web_search_tool` returns `(tool_result_text, queries_charged)` tuple
...
- [ ] **1d.** Update `_RESEARCHER_SYSTEM_PROMPT` with batch query documentation and first-turn guidance
...
- [ ] **1e.** Add unit tests for batch query flow
