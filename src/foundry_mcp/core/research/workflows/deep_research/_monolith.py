...
from foundry_mcp.core.research.workflows.deep_research._budgeting import (
    allocate_source_budget as _allocate_source_budget_standalone,
    allocate_synthesis_budget as _allocate_synthesis_budget_standalone,
    archive_digest_source as _archive_digest_source_standalone,
    compute_refinement_budget as _compute_refinement_budget_standalone,
    extract_report_summary as _extract_report_summary_standalone,
    final_fit_validate as _final_fit_validate_standalone,
    summarize_report_for_refinement as _summarize_report_for_refinement_standalone,
    write_digest_archive as _write_digest_archive_standalone,
    validate_archive_source_id as _validate_archive_source_id_standalone,
    cleanup_digest_archives as _cleanup_digest_archives_standalone,
    ensure_private_dir as _ensure_private_dir_standalone,
)
...
class DeepResearchWorkflow(PlanningPhaseMixin, GatheringPhaseMixin, ResearchWorkflowBase):
...
IMPORTANT: Return ONLY valid JSON, no markdown formatting or extra text."""
    ...
    def _archive_digest_source(self, *, source, digestor, raw_content, page_boundaries, source_text_hash):
        """Archive canonical text for a digested source."""
        return _archive_digest_source_standalone(
            source=source, digestor=digestor, raw_content=raw_content,
            page_boundaries=page_boundaries, source_text_hash=source_text_hash,
            retention_days=self.config.deep_research_archive_retention_days,
        )

    def _write_digest_archive(self, *, source_id, source_text_hash, canonical_text, retention_days):
        """Write canonical text to the digest archive directory."""
        return _write_digest_archive_standalone(
            source_id=source_id, source_text_hash=source_text_hash,
            canonical_text=canonical_text, retention_days=retention_days,
        )

    def _validate_archive_source_id(self, source_id):
        """Validate source_id is safe to use as an archive path component."""
        return _validate_archive_source_id_standalone(source_id)

    def _cleanup_digest_archives(self, source_dir, retention_days):
        """Remove archived digest files older than retention_days."""
        return _cleanup_digest_archives_standalone(source_dir, retention_days)

    def _ensure_private_dir(self, path):
        """Ensure directory exists with owner-only permissions."""
        return _ensure_private_dir_standalone(path)

    def _allocate_source_budget(self, state, provider_id):
        """Allocate token budget across sources for analysis phase."""
        return _allocate_source_budget_standalone(state, provider_id)
    ...
    def _allocate_synthesis_budget(self, state, provider_id):
        """Allocate token budget for synthesis phase."""
        return _allocate_synthesis_budget_standalone(state, provider_id)

    def _compute_refinement_budget(self, provider_id, state):
        """Compute token budgets for refinement phase."""
        return _compute_refinement_budget_standalone(provider_id, state)

    def _summarize_report_for_refinement(self, report, target_tokens):
        """Summarize report content to fit within token budget."""
        return _summarize_report_for_refinement_standalone(report, target_tokens)

    def _extract_report_summary(self, report, char_limit):
        """Extract summary from report preserving structure."""
        return _extract_report_summary_standalone(report, char_limit)

    def _final_fit_validate(self, system_prompt, user_prompt, provider_id, model, output_reserved, phase):
        """Validate assembled payload fits within context budget."""
        return _final_fit_validate_standalone(system_prompt, user_prompt, provider_id, model, output_reserved, phase)
