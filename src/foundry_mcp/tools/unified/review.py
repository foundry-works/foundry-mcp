...
from foundry_mcp.core.spec import find_spec_file, find_specs_directory, load_spec, update_frontmatter
...
def _handle_spec_review(*, config: ServerConfig, payload: Dict[str, Any]) -> dict:
    ...
    if result.get("success") and not dry_run and specs_dir:
        ...
        try:
            ...
            # Write spec_review_path to spec metadata for future discovery
            rel_path = f".spec-reviews/{spec_id}-spec-review.md"
            try:
                update_frontmatter(spec_id, "spec_review_path", rel_path, specs_dir=specs_dir)
            except Exception as fm_exc:
                logger.warning("Failed to update spec frontmatter with review path: %s", fm_exc)
