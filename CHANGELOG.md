...
- **Academic profiles now include OpenAlex and Crossref**: Built-in academic profiles (`academic`, `systematic-review`, `bibliometric`) hardcoded only `semantic_scholar` and `tavily` as providers, so OpenAlex and Crossref were never activated even with credentials configured. All academic profiles now default to all four providers.
- **Sample configs updated**: Quick-start and reference TOML samples now include `openalex` and `crossref` in the providers list with credential documentation.
...
- **Promote extraction and token deps to core**: `python-docx`, `pdfminer.six`, `pypdf`, and `tiktoken` are now core dependencies instead of optional extras. PDF/DOCX extraction and accurate token counting are integral to deep research workflows and should work out of the box. Removed `[pdf]`, `[docx]`, and `[tokens]` optional dependency groups.

## [0.17.0b6] - 2026-02-28

### Added

- **DOCX extraction**: New `DocxExtractor` with security-hardened DOCX-to-markdown conversion. Includes content classifier for binary content detection, SSRF protection, and ZIP bomb guards. `python-docx` added as a core dependency.
...
- **Query clarification phase**: `CLARIFICATION` phase runs before planning to analyze query completeness. Sends the query to a fast model that identifies whether clarification is needed and infers constraints for the planning phase. Enabled by default (`deep_research_allow_clarification = true`).
...
- **BREAKING**: Invalid server roles now fall back to `observer` (fail-closed) instead of `maintainer`. If your configuration contains a typo in the role name, the server will now start in read-only mode rather than full-access mode. Verify your `FOUNDRY_SERVER_ROLE` value is valid.
...
- **Date-suffixed plan filenames**: `plan action="create"` now appends a UTC date to the plan slug (e.g. `feature-name-2026-02-22.md`), mirroring the spec_id date convention. Plan review files inherit the dated name automatically (`feature-name-2026-02-22-review.md`).
...
- **Plan review synthesis severity inflation**: Synthesis prompt now enforces severity fidelity rules — issues cannot be escalated beyond the highest severity any individual reviewer assigned. Added "Minor Suggestions" and "Escalation Candidates" sections to the synthesis output format.
- **Authoring tool `plan_path`/`plan_review_path` passthrough**: Added missing `plan_path` and `plan_review_path` parameters to the unified authoring tool signature so `spec-create` can receive them from LLM callers.
...
- **Autonomous Spec Execution Engine (ADR-002)**: Durable session management for autonomous spec-driven task execution with replay-safe semantics.
  ...
  - Fidelity gates: per-phase quality gates with strict/lenient/manual policies, auto-retry with cycle caps, tiebreaker reviewer logic, and human acknowledgment flow
  - Write-lock enforcement with server-side task completion persistence
  ...
  - Spec integrity validation: structure hashing with mtime fast-path, rebase recovery for mid-session spec edits
  - Autonomy posture profiles (`unattended`, `supervised`, `debug`) with runtime defaults
  - Server-secret HMAC authorization, comprehensive audit logging, bash/write guard scripts
  - Spec adapter bridging hierarchy-based specs to the phases-array view expected by the orchestrator
  - Stale session recovery: replay-based probe for unreported pending steps with proof reissuance on crash recovery
  - Phantom task reconciliation (Step 4b): cross-checks completions against spec truth, self-healing revocation
  - `foundry-implement-auto` skill with machine-parseable JSON exit payloads for supervisor handoff
- **Foundry CLI**: `foundry run`, `foundry stop`, `foundry watch` commands with shell injection prevention, shared signal logic, and cross-platform guards
- **Spec metadata fields**: `success_criteria`, `constraints`, `risks`, `open_questions` with CRUD handlers, validation, and duplicate detection
- **Task complexity**: `low`/`medium`/`high` field with validation and hierarchy support
- **Plan linkage**: Required `plan_path` and `plan_review_path` on spec creation with file-existence validation
- **Spec-vs-plan review**: Automatic comparison review when a spec has a linked plan, with few-shot semantic matching calibration
- **Verification scaffolding**: Auto-appended by `add_phase_bulk()` (run-tests + fidelity-verify nodes with deduplication)
- **Static analysis tooling**: ruff (format + lint), pyright with CI ratchet workflow, pre-commit config, Makefile shortcuts
- **Declarative parameter validation** (`param_schema.py`): Schema-driven validation for all 69 handlers, reducing ~370 imperative calls to 4

### Changed

- **Rebrand SDD → Foundry**: All references, env vars (`SDD_` → `FOUNDRY_`), CLI commands, skill prefixes, and documentation renamed
- **Default server role**: `observer` → `maintainer` for interactive use; autonomous sessions use posture-driven overrides
- **Type safety**: Fixed all 573 pyright errors to zero across the codebase
- **Error hierarchy** (`core/errors/`): Consolidated 37 error classes from 17 files into 9 domain modules with `ERROR_MAPPINGS` registry
- **Module decompositions** (all with backward-compatible re-exports):
  - `core/validation/` (2,342 → 10 sub-modules), `core/ai_consultation/` (1,774 → 4), `core/observability/` (1,218 → 6)
  - `core/discovery/` (1,811 → 10), `core/llm_config/` (1,419 → 5), `core/responses/` (1,697 → 7)
  - `config/` (2,771 → 7), `core/research/models/` (2,176 → 8), `core/autonomy/models/` (1,154 → 8)
  - Research infrastructure: 7 modules (~9,936 lines) → `document_digest/`, `context_budget/`, `summarization/`, `token_management/`, `providers/resilience/`, deep research core extraction
- **Handler decompositions**: Research (17 handlers → 5 domain modules), task/authoring/spec/environment/review routers split into handler packages
- **Tool registration**: ~470 lines of manual payload construction replaced with `locals()` filter pattern
- **Deep research lifecycle**: Shared LLM call boilerplate extracted from 4 phase mixins (~600 lines eliminated)
- **Test fixtures**: 5 `conftest.py` files centralizing ~820 lines of duplicates
- **Plan templates**: Merged simple/detailed into a single spec-aligned template; removed review type variants
- **Fidelity review context**: Now includes task descriptions, categories, phase purpose, spec overview, subsequent phases, acceptance criteria, plan content; scoped test results and journal entries to task/phase under review; compact path listing instead of inline file content
- **Fidelity review accuracy**: Explicit verdict criteria, severity-based tiebreaking, tiebreaker reviewer for split verdicts, self-referential node filtering

### Removed

- **Feature flag system**: Entire infrastructure removed; autonomy features always enabled
- **Sub-phases, phase dependencies, phase templates, bulk phase operations**: Removed deprecated spec system complexity
- **Spec/phase-level `estimated_hours` and `instructions`**: Removed from spec/phase metadata (retained at task level)

### Fixed

- **Autonomy engine stability** (22 fixes across the beta cycle):
  - Evidence lifecycle: gate reviewer receives full journal content, test results, and verification receipts
  - Write-lock completion deadlock: server-side persistence when runner cannot write
  - Hierarchy-blind lookups: checks hierarchy dict first, phases-array fallback
  - Proof deadlocks: identity pre-check before token consumption; reissuance on crash recovery
  - Phase completion: no longer triggers false `BLOCKED` pause on dependency-blocked subsequent phases
  - Phantom task reconciliation: self-healing revocation of completions not reflected in spec
  - Session reuse safety: rejects sessions paused at fidelity cycle limit
  - Gate evidence deferral: prevents re-emission while pending evidence awaits processing
  - Agent-facing UX: aggregated validation errors, outcome hints in step messages, protected step proofs

### Migration

- **Legacy session action names remain supported** but are on an explicit removal window: **3 months or 2 minor releases (whichever is later)**.
- Legacy action responses include machine-readable deprecation metadata (`meta.deprecated.action`, `meta.deprecated.replacement`, `meta.deprecated.removal_target`).
- All environment variables renamed from `SDD_*` / `SDD_MCP_*` to `FOUNDRY_*` / `FOUNDRY_MCP_*`.
- All backward-compatible `__init__.py` re-exports preserved for decomposed packages.
