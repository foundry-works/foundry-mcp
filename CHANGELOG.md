...
- **Query clarification phase**: Optional `CLARIFICATION` phase runs before planning to analyze query completeness. Sends the query to a fast model that identifies whether clarification is needed and infers constraints for the planning phase. Opt-in via `deep_research_allow_clarification = true`.
