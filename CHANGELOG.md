...
- **`foundry run` blank agent pane**: tmux spawns non-interactive shells that don't load aliases or the full user PATH. Now resolves absolute paths for `claude` and `foundry` binaries before passing commands to tmux.
